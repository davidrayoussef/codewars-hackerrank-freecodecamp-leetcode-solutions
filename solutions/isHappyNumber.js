/*
Write an algorithm to determine if a number is "happy". A happy number is a number defined by the following process:
Starting with any positive integer, replace the number by the sum of the squares of its digits, and repeat the process
until the number equals 1 (where it will stay), or it loops endlessly in a cycle which does not include 1.
Those numbers for which this process ends in 1 are happy numbers.

Example: 19 is a happy number
12 + 92 = 82
82 + 22 = 68
62 + 82 = 100
12 + 02 + 02 = 1
*/

function isHappyNumber(n) {
  const splitNumbers = (n) => String(n).split('').map(Number);
  const sumSquares = (nums) => nums.map(v => v ** 2).reduce((acc,curr) => acc + curr);
  let result = 0;
  let record = [];

  while (result !== 1 && !record.includes(result)) {
    result = sumSquares(splitNumbers(n));
    record.push(n);
    n = result;
  }

  return result === 1;
}

isHappyNumber(19); //=> true
isHappyNumber(20); //=> false
